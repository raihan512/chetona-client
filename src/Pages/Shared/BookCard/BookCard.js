...
const BookCard = ({ book }) => {
    ...
    return (
        <div className='w-[310px] sm:w-[350px] h-[150px] sm:h-[180px] rounded-sm border border-[#40A4DC] box-border flex justify-between items-center mb-10 transition-all hover:scale-110'>
            {/* Book & Author Image */}
            <div className='w-5/12 h-[155px] sm:h-[175px] -mt-12 ml-2'>
                <div className='h-[155px] sm:h-[175px] relative'>
                    {/* Book */}
                    <img src={img} className="h-full min-w-[100px] border border-[#40A4DC] bg-[#F6F6F6]" alt={`${title}`} />
                    {/* Author */}
                    <img src={`${authorImg ? authorImg : 'https://i.ibb.co/cJSy10s/user-50-X50.png'}`} className='absolute -bottom-3 sm:-bottom-5 left-1/4 h-[35px] sm:h-[50px] w-[35px] sm:w-[50px] bg-slate-300 rounded-full border border-[#40A4DC]' alt={`${author}`} />
                </div>
            </div>
